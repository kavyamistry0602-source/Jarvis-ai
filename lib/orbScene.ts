...
export function createOrbScene(container: HTMLElement): OrbSceneApi {
  ...
  const bloom = new UnrealBloomPass(
    ...
    2.0, // cyan glow strength
    0.55, // wider holographic bloom
    0.12, // lower threshold for fine cyan details
  );
  ...
  const chromaticShader = {
    ...
    fragmentShader: `
      ...
      void main() {
        ...
        // JARVIS cyan/blue holographic color grade
        gl_FragColor.rgb = mix(
          gl_FragColor.rgb,
          gl_FragColor.rgb * vec3(0.65, 1.05, 1.25),
          0.28
        );
      }
    `,
  ...
  // ——— COLORS ———
  const C_BRIGHT = 0x00d9ff;
  const C_MID = 0x0088cc;
  const C_DIM = 0x005577;
  const C_FAINT = 0x003344;
  const C_HOT = 0x66eeff;
  ...
  function makeTextSprite(text: string, size = 0.08) {
    ...
    ctx.fillStyle = `rgba(${Math.min(255, 120 + Math.random() * 100) | 0}, ${Math.min(255, 210 + Math.random() * 45) | 0}, 255, ${alpha})`;
  ...
  g.addColorStop(0, "rgba(102,238,255,1)");
  g.addColorStop(0.2, "rgba(0,217,255,0.65)");
  g.addColorStop(0.5, "rgba(0,136,204,0.18)");
  g.addColorStop(1, "rgba(0,51,68,0)");
  dCtx.fillStyle = g;
  ...
  function animate() {
    ...
    // Bloom pulse
    bloom.strength = 1.8 + Math.sin(t * 0.8) * 0.35;
